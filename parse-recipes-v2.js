...
const CATEGORY_MAP = {
  ...
  'desert': '56230a66-83f0-4a25-9a50-8bfd17d08448',  // Desserts
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      ...
      if (item) {
        ...
        // Common pattern: "1½ cups flour" or "1/4 teaspoon salt" or "2 large eggs"
        const match = item.match(/^(?:Recipe\s*)?([\d\s\./½¼¾⅓⅔⅛⅜⅝-]+)?\s*(cup|cups|tbsp|tablespoon|tablespoons|tsp|teaspoon|teaspoons|oz|ounce|ounces|lb|pound|pounds|g|gram|grams|kg|package|can|piece|pieces|slice|slices|clove|cloves|large|small|medium|stick|sticks|bottle|bottles|box|boxes|envelope|envelopes|jar|jars)?\.?\s*(.+)?$/i);
...
function parseSimpleRecipe(content, filePath) {
  ...
  // Dessert detection
  const dessertWords = ['pie', 'cake', 'cookie', 'muffin', 'scone', 'biscotti', 'fudge', 'candy', 'custard', 'cheesecake', 'tart', 'brownie', 'frosting', 'shortcake', 'tiramisu'];
...
function main() {
  ...
  const emptyTitles = recipes.filter(r => !r.title);
  if (emptyTitles.length > 0) {
    console.log(`\n=== ${emptyTitles.length} recipes with empty titles ===`);
    emptyTitles.slice(0, 5).forEach(r => console.log(`  - ${r.file_path}`));
  }
}
